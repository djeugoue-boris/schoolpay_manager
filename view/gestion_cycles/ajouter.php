...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO cycles (nom_cycle) VALUES (?)");
            $stmt->execute([$nom_cycle]);
            header("Location: index.php?message=Cycle ajouté avec succès");
            exit();
        } catch (PDOException $e) {
            $errors[] = "Erreur lors de l'ajout du cycle: " . $e->getMessage();
        }
    }
}
?>
            ...
            <form method="POST">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nom_cycle" name="nom_cycle" 
                           value="<?= htmlspecialchars($_POST['nom_cycle'] ?? '') ?>" 
