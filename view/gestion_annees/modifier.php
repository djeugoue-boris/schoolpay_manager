...
if (!$annee) {
    $_SESSION['message'] = "Erreur : Année scolaire introuvable.";
    header('Location: index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Vérifier si l'année existe déjà (sauf pour l'année actuelle)
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM annees_scolaires WHERE libelle = ? AND id != ?");
    $stmt->execute([$libelle, $id]);
    
    if ($stmt->fetchColumn() > 0) {
        $_SESSION['message'] = "Erreur : Cette année scolaire existe déjà.";
    } else {
        ...
        $stmt = $pdo->prepare("UPDATE annees_scolaires SET libelle = ?, active = ? WHERE id = ?");
        $stmt->execute([$libelle, $active, $id]);
        
        $_SESSION['message'] = "Année scolaire modifiée avec succès.";
        header('Location: index.php');
        exit;
    }
}
...
<div class="dropdown">
  ...
  <div class="collapse ps-3" id="menuParam">
    <a href="index.php" class="d-block mt-2"><i class="bi bi-calendar"></i> Config Années Scolaires</a>
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title text-dark fw-bold text-center">Modifier l'année scolaire</h3>
                    </div>
                    <div class="card-body">
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                <label for="libelle" class="form-label text-dark">Libellé de l'année scolaire</label>
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="active" name="active" 
                                           <?php echo $annee['active'] ? 'checked' : ''; ?>>
                                    <label class="form-check-label text-dark" for="active">
